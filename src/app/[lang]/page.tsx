  ...
  return (
    <div className="flex min-h-screen items-center justify-center">
      <div className="rounded-2xl border border-zinc-200 bg-white p-10 text-center shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <h1 className="mb-6 text-3xl font-bold">OurWay</h1>
        <div className="flex items-center justify-center gap-3">
          ...
          <span className="text-zinc-600 dark:text-zinc-400">
            ...
            <span className={ok ? "text-green-600" : "text-red-500"}>
              {ok ? data?.status : dict.common.error}
            </span>
          </span>
        </div>
        <p className="mt-4 text-sm text-zinc-400">{process.env.NEXT_PUBLIC_API_URL}</p>
      </div>
    </div>
  );
}
